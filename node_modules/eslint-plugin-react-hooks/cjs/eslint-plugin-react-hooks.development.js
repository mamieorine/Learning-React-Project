...
function isDefinitelyStaticDependency(reference) {
  ...
  if (callee.name === 'useRef' && id.type === 'Identifier') {
    ...
    if (id.type === 'ArrayPattern' && id.elements.length === 2 && Array.isArray(reference.resolved.identifiers) &&
    ...
    function visitFunctionExpression(node) {
        ...
        try {
          for (var _iterator = currentScope.references[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            if (!dependencies.has(dependency)) {
              var isStatic = isDefinitelyStaticDependency(reference);
      ...
      if (declaredDependenciesNode.type !== 'ArrayExpression') {
        ...
        context.report({
          ...
          message: 'React Hook ' + context.getSource(reactiveHook) + ' has a second ' + "argument which is not an array literal. This means we can't " + "statically verify whether you've passed the correct dependencies."
        });
      } else {
      ...
      var actualDependencies = Array.from(dependencies.keys());
      var foundStaleAssignments = false;
      ...
      declaredDependencies.forEach(function (_ref) {
        var key = _ref.key;

        if (actualDependencies.some(function (actualDep) {
          return satisfies(actualDep, key);
        })) {
          // Legit dependency.
      ...
      dependencies.forEach(function (_ref2, key) {
        ...
        if (reference.writeExpr) {
          ...
          context.report({
            ...
            message: 'Assignments to the \'' + key + '\' variable from inside a React ' + context.getSource(reactiveHook) + ' Hook ' + 'will not persist between re-renders. ' + 'If it\'s only needed by this Hook, move the variable inside it. ' + 'Alternatively, declare a ref with the useRef Hook, ' + 'and keep the mutable value in its \'current\' property.'
          });
        }

        if (!suggestedDependencies.some(function (suggestedDep) {
          return satisfies(key, suggestedDep);
        })) {
          if (!isStatic) {
            // Legit missing.
            suggestedDependencies.push(key);
      ...
      if (unnecessaryDependencies.size > 0) {
        ...
        if (badRef !== null) {
          extraWarning = ' Mutable values like \'' + badRef + '\' aren\'t valid dependencies ' + "because their mutation doesn't re-render the component.";
        }
      }
...
function fastFindReferenceWithParent(start, target) {
  ...
  while (queue.length) {
    ...
    try {
      for (var _iterator3 = Object.entries(item)[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
